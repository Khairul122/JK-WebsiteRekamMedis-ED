...
if (isset($_POST['add'])) {
    ...
    $lama_nginap = trim(mysqli_real_escape_string($con, $_POST['lama_nginap']));
    $status = trim(mysqli_real_escape_string($con, $_POST['status']));
    
    // Calculate biaya based on status
    if ($status === 'BPJS') {
        $biaya = 0;
    } elseif ($status === 'Mandiri') {
        $biaya = 100000 * $lama_nginap;
    } else {
        $biaya = 0;
    }
    ...
    if (mysqli_num_rows($sql_cek_identitas) > 0) {
        ...
        // Insert data into tb_pasien
        mysqli_query($con, "INSERT INTO tb_pasien (nomor_identitas, nama_pasien, jenis_kelamin, alamat, no_telp,lama_nginap, status, biaya) 
                             VALUES ('$identitas', '$nama', '$jk', '$alamat', '$telp','$lama_nginap', '$status', '$biaya')") or die(mysqli_error($con));

        // Get the last inserted ID (assuming id_pasien is an auto-incremented primary key)
        $id_pasien = mysqli_insert_id($con);
    ...
    $sql = substr($sql, 0, -1);
    // echo $sql;
    mysqli_query($con, $sql) or die(mysqli_error($coon));
    unlink($target_file);
    echo "<script>window.location='data.php';</script>";
}
